...
Linux 内核开源项目有着为数众广的参与者。绝大多数的 Linux 内核维护工作都花在了提交补丁和保存归档的繁琐事务上(1991－2002年间)。到 2002 年，整个项目组开始启用一个专有的分布式版本控制系统 商业公司BitKeeper 来管理和维护代码。
...
- Remote：远程仓库，托管代码的服务器，可以简单的认为是你项目组中的一台电脑用于远程数据交换（github）
...
```
# 进入项目目录
cd C:/Users/13901/Desktop/test/demo1/
...
# 添加远程仓库
git remote add origin https://github.com/nahannb/test.git
...
- 将本地的 `test` 分支推送到远程仓库的 `test` 分支。
- 同时设置本地的 `test` 分支跟踪远程的 `origin/test` 分支，这样以后您可以直接使用 `git push` 和 `git pull` 而不需要每次指定远程和分支名。
